...
const changePassword = async (req, res) => {
    try {
        ...
        if (!user) {
            return res.status(404).json({ success: false, error: "user not error" })
        }
...
export const updateProfileImage = async (req, res) => {
    try {
        ...
        return res.status(200).json({ success: true, message: "Profile image updated successfully", profileImage: req.file.path });
    } catch (error) {
        return res.status(500).json({ success: false, error: "Server error updating profile image" });
    }
};